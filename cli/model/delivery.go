...
type Delivery struct {
	...
	DeliveryStatus        enum.DeliveryStatusEnum `gorm:"column:delivery_status"`
...
func FakeDelivery(f faker.Faker) Delivery {
	...
	status := enum.RandomEnum[enum.DeliveryStatusEnum](f, enum.DeliveryStatusEnum("").Values())

	// Set delivery_date only when status is DeliveryDelivered or DeliveryFailed
	if status == enum.DeliveryDelivered || status == enum.DeliveryFailed {
