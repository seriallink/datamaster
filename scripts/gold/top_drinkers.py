...
class Processor(BaseProcessor):
    """
    Processor for generating the 'top_drinkers' table in the gold layer.

    This processor reads review_flat data from the silver layer, aggregates
    review counts and average ratings by user (profile_id) and writes the result.
    """
    ...
    def run(self):
        try:
            ...
            logger.exception(f"[{self.table}] Failed to process top drinkers")
            raise
