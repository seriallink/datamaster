...
class Processor(BaseProcessor):
    """
    Processor for generating the 'top_breweries_by_rating' table in the gold layer.

    This processor reads review_flat data from the silver layer, aggregates
    brewery ratings by month, and writes the top breweries by average overall score.
    """
    ...
    def run(self):
        try:
            ...
            logger.exception(f"[{self.table}] Failed to process top breweries by rating")
            raise
